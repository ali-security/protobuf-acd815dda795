...
UPB_INLINE void* UPB_PRIVATE(_upb_Arena_FindAndPopPoolBlock)(
  ...
  int start_bin = 0;
  if (span > 16) {
    start_bin = upb_Log2Floor(span - 1) + 1 - 4;
  }
  ...
  if (host->UPB_PRIVATE(bins)[start_bin] != NULL) {
    UPB_PRIVATE(_upb_ArenaFreeBlock)* block =
        host->UPB_PRIVATE(bins)[start_bin];
    host->UPB_PRIVATE(bins)[start_bin] = block->UPB_PRIVATE(next);
    *actual_size = (size_t)1 << (start_bin + 4);
    return block;
  }

  // 2. Search for the smallest non-empty larger bin (excluding the host block
  // itself)
  for (int i = start_bin + 1; i < num_bins; ++i) {
    if (host->UPB_PRIVATE(bins)[i] != NULL) {
      UPB_PRIVATE(_upb_ArenaFreeBlock)* block = host->UPB_PRIVATE(bins)[i];
      host->UPB_PRIVATE(bins)[i] = block->UPB_PRIVATE(next);
      *actual_size = (size_t)1 << (i + 4);
      return block;
    }
  }

  // 3. No blocks in host->bins[start_bin .. num_bins - 1].
  // Only the host block itself can satisfy the request!
  // Evacuate the array holder by scanning backwards for the first non-null
  // entry.
  int next_host_bin = -1;
...
UPB_API_INLINE void* upb_Arena_AllocPool(struct upb_Arena* a,
  ...
  if (UPB_PRIVATE(_upb_Arena_IsValidPoolSize)(pool_size)) {
